...
export function fragmentForPreview(html, partId, localAssets = false) {
  const parsed = new DOMParser().parseFromString(html, 'text/html');
  parsed.querySelectorAll('script,style,link,meta,title,base,iframe,object,embed,noscript,template').forEach(el=>el.remove());
  for(const el of parsed.body.querySelectorAll('*')) {
    for(const attr of [...el.attributes]) {
      if (/^on/i.test(attr.name) || attr.name==='style' || attr.name==='srcdoc' || attr.name.startsWith('x-') || attr.name.startsWith('data-on') || ['action','formaction','autofocus','srcset'].includes(attr.name)) el.removeAttribute(attr.name);
  ...
  const fragment=document.createDocumentFragment();fragment.append(...root.childNodes);return fragment;
}
export async function initPreview() {
 ...
 for(const module of view.modules.filter(m=>selected.has(m.partId))) {
  ...
  if(module.needsCapture) {
    const placeholder=document.createElement('div');placeholder.className='sim-widget-placeholder';
    const title=document.createElement('strong');title.textContent=module.type;
    const text=document.createElement('p');text.textContent='Rendered markup needed. Inspect this part in Slate and add a local capture.';
    placeholder.append(title,text);part.append(placeholder);
  } else {
    const content=fragmentForPreview(module.html,module.partId,Boolean(module.replacement && !module.hasFixture));
 ...
 if(clean) {const app=document.createElement('script');app.src='/js/app.js';document.body.append(app);}
}
