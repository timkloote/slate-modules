...
async function initSimulator() {
  ...
  for (const module of view.modules) {
    ...
    const fields = {
      ...
      'Local source':module.source||'No source file', 'Capture file':module.fixture,
      'Preview substitution':module.replacement ? `Shared ${module.replacement}; original source retained` : 'None',
    };
    ...
    sourceTitle.textContent='Original HTML'; pre.textContent=module.sourceHtml||'(no source)'; source.append(sourceTitle,pre);details.append(source);
    ...
    card.append(label,meta,details);$('module-list').append(card);cards.push({card,check,module});
  }
  function filter() {
    const query=$('module-search').value.toLowerCase(), mode=$('module-filter').value;
    for(const {card,module:m} of cards) card.hidden= !`${m.name} ${m.partId} ${m.type} ${m.notes}`.toLowerCase().includes(query) || (mode==='capture'&&!m.needsCapture) || (mode==='active'&&m.status!=='Active') || (mode==='inactive'&&m.status!=='Inactive');
  }
  ...
  function updatePreview() {
    try {
      ...
      localStorage.setItem(settingsKey,JSON.stringify({'layout-mode':$('layout-mode').value,'class-mode':$('class-mode').value}));
    } catch {}
    const params=new URLSearchParams({parts:[...selected].join(','),classes:$('class-mode').value,layout:'stacked',labels:$('show-labels').checked?'1':'0'});
  ...
  for(const id of ['class-mode','layout-mode','show-labels'])$(id).addEventListener('change',updatePreview);
